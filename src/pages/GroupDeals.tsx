...
import { Users, Clock, ArrowLeft, Plus, Copy, Share2, Timer, Check } from "lucide-react";
...
const GroupDeals = () => {
  ...
  const [dealParticipants, setDealParticipants] = useState<Record<string, any[]>>({});
  ...
  useEffect(() => {
    const channel = supabase.channel("group-deals-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "group_deal_participants" }, () => fetchDeals())
      .subscribe();
  ...
  const fetchDeals = async () => {
    const { data: dealsData } = await supabase.from("group_deals").select("*, products(*)").eq("is_active", true).order("created_at", { ascending: false });
    if (dealsData) {
      setDeals(dealsData);
      const participantsMap: Record<string, any[]> = {};
      for (const deal of dealsData) {
        const { data: parts } = await supabase.from("group_deal_participants").select("*").eq("deal_id", deal.id);
        participantsMap[deal.id] = parts || [];
      }
      setDealParticipants(participantsMap);
    }
  };
  ...
  const createDeal = async () => {
    ...
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); }
      ...
      // Auto-join creator
      const { data: newDeal } = await supabase.from("group_deals").select("*").eq("invite_code", code).single();
      if (newDeal) {
        await supabase.from("group_deal_participants").insert({ deal_id: newDeal.id, user_id: user.id });
      }
      setShowCreate(false);
      setSelectedProduct("");
      fetchDeals();
      toast({ title: "Deal created!", description: `Share code: ${code}` });
    }
    setLoading(false);
  };
  ...
  const joinDeal = async (dealId: string) => {
    ...
    if (error && error.code === "23505") { toast({ title: "Already joined", description: "You're already part of this deal." }); }
    else if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); }
    else { toast({ title: "Joined! 🎉", description: "You've joined the group deal." }); fetchDeals(); }
  };

  const joinDealByCode = async (code: string) => {
    if (!user) { navigate("/auth"); return; }
    const { data: deal } = await supabase.from("group_deals").select("*").eq("invite_code", code.toUpperCase()).eq("is_active", true).single();
    if (deal) joinDeal(deal.id);
    else toast({ title: "Deal not found", variant: "destructive" });
  };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      {products.map((p) => (
                        <button key={p.id} onClick={() => setSelectedProduct(p.id)}
                          className={`rounded-lg border overflow-hidden transition-all ${selectedProduct === p.id ? "border-primary ring-2 ring-primary/30" : "border-border"}`}>
                  ...
                  {selectedProduct && (
                    <div className="bg-secondary/50 rounded-lg p-3 font-body text-sm">
                      <p>Solo price: <span className="text-foreground font-semibold">${products.find((p) => p.id === selectedProduct)?.price}</span></p>
                      <p>Group price: <span className="text-primary font-semibold">${Math.round(products.find((p) => p.id === selectedProduct)?.price * (1 - discountPercent / 100))}</span>
          ...
          <div className="space-y-4">
            {deals.length === 0 ? (
              ...
              deals.map((deal) => {
                const parts = dealParticipants[deal.id] || [];
                ...
                const hasJoined = parts.some((p) => p.user_id === user?.id);
                    ...
                    <Card className={`border-border bg-card overflow-hidden ${isExpired ? "opacity-60" : ""}`}>
                      <div className="flex flex-col sm:flex-row">
                        ...
                        <div className="flex-1 p-5">
                          ...
                          {!isExpired && !hasJoined && (
                            <Button onClick={() => joinDeal(deal.id)} className="bg-gradient-gold text-primary-foreground font-body tracking-wider uppercase text-xs">
