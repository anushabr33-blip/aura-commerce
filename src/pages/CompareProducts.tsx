                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="p-4 text-left font-body text-sm text-muted-foreground uppercase tracking-wider w-40">Spec</th>
                        {selected.map((product) => (
                          <th key={product.id} className="p-4 text-center min-w-[180px]">
                            <img src={getProductImage(product.image_url)} alt={product.name} className="w-20 h-20 object-cover rounded-lg mx-auto mb-2" />
                    ...
                    <tbody>
                      ...
                      <tr className="border-b border-border">
                        ...
                        {selected.map((product) => (
                          <td key={product.id} className="p-4 text-center font-body text-sm">{product.category}</td>
                        ))}
                      </tr>
                      {allSpecKeys.map((key) => (
                        <tr key={key} className={`border-b border-border transition-colors ${isDifferent(key) ? "bg-primary/5" : ""}`}>
                          ...
                          {selected.map((product) => (
                            <td key={product.id} className={`p-4 text-center font-body text-sm ${isDifferent(key) ? "font-semibold text-foreground" : "text-muted-foreground"}`}>
